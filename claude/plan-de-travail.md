...
## 2 bis — 🔴 **D8 / D10 — le plafond de ≈ 4,5 Mo : l'écart concurrentiel réel**
...
- **Mécanisme retenu le 20 septembre (résout D8 ET le bloquant 6) :** billet signé émis par Vercel + **envoi direct par morceaux vers le service Railway** — c'est ce que FreeConvert fait. **Construit, déployé et prouvé pour la vidéo (bloquant 6). Branchement des routes Office NON fait : 8-12 h, et il est confirmé nécessaire.** ✅ **Contradiction TRANCHÉE le 20 septembre par mesure en production** (route `/api/convert-to-pdf`, fichier `.txt` refusé par l'application avant tout quota) : 1 Mio et 4 Mio passent (400 applicatif), **4 493 821 octets acceptés, 4 493 924 refusés** (`FUNCTION_PAYLOAD_TOO_LARGE`, plafond ≈ 4,5 Mo de corps de requête), et **tout ce qui est plus gros est refusé** (4,4 Mio, 6, 10, 26, 60, 100 Mio). **La note « Vercel accepte 100 Mo » est FAUSSE pour ce projet.** ⚠️ Le commentaire de `app/api/convert-to-pdf/route.ts` (« Vercel Functions accept request bodies up to 100 MB… ») est faux et à corriger lors du chantier D8 ; le plafond applicatif de 25 Mo y est inatteignable.
...
**Prouvé** (Chromium en Preview ET en production, Firefox en Preview ; vraies pages, vrai service, fichiers rouverts par ffmpeg : durées exactes, 0 erreur) : compresser/convertir 30 s (1080p, 21,7 Mo) en **≈ 10-20 s** de bout en bout, 3 min (720p, 71,8 Mo) en **≈ 26-30 s**, téléversement compris, progression réelle (12 à 70 valeurs distinctes affichées). Réveil après veille : **+1,05 s** en HTTP direct (1,23 s contre 0,17 s), **+2,6 s** vu de la page (« Preparing… » affiché). **Non prouvé : Safari réel / iPhone, WebKit de Playwright** (bloquant 9). Mesures faites depuis une connexion de bureau rapide.

**Coût mensuel retenu : 0,00 $ à trafic nul, ≈ 1,2 $ à 500 conversions/mois, ≈ 12 $ à 5 000** (Railway, facturation à la seconde). **À confirmer sur la première facture réelle** — non mesuré ici.

**Niveau atteint face au marché (20/09/2026, MP4 → WebM, même vidéo, sans compte)** : **plus rapide chez tous les concurrents mesurés**. 30 s : nous **18,0 s** (47,0 Mo, VMAF 79,9) · Online-Convert ≈ 100 s (sortie 9,9 Mo, VMAF 68,2 — réglages plus légers) · FreeConvert 203 s (44,1 Mo ; **nous : 47,0 Mo, soit 6,6 % de plus** — erreur d'unités corrigée le 20/09 : « 44,8 » était en Mio) · Convertio 251 s (117 Mo) · Zamzar non fini à 400 s. 3 min : nous **43,7 s** · FreeConvert 223 s · Online-Convert non fini à 500 s. 123apps non mesurable (interstitiel publicitaire bloquant). Plafond **1 Go** (Convertio annonce 1 Go, FreeConvert 1 Go, Zamzar 50 Mo), sans filigrane ni compte. **Retard à assumer : la couverture de formats** — 19 sorties chez nous, des centaines chez Convertio/Zamzar (pas de H.265/HEVC, AV1, 3G2, ASF, VOB, M2TS, AAC/AIFF/WMA/AC3/AMR en sortie). Détail : `RAPPORT-video-deploiement.md`.

**Reste** : ① Safari réel (macOS, iPhone) sur les deux outils vidéo, `image-converter` (AVIF), `video-trimmer` ; ② couverture de formats (H.265 en premier) ; ③ première facture Railway ; ④ branchement Office (D8) — voir 2 bis.
...
| **Juste APRÈS** | **Relever le plafond de ≈ 4,5 Mo** (D8) · **Vague 5 de l'audit** · **les deux réserves du réviseur** (IP falsifiable, compensation non atomique) · **purge de `tool_errors`** au-delà de 90 jours |
...
| **Si les refus à ≈ 4,5 Mo apparaissent dans `tool_errors`** | Le chantier D8 passe **avant** les autres — c'est une demande réelle mesurée. |
...
- **Le plafond de charge utile des fonctions Vercel est de ≈ 4,5 Mo.** **Mesuré le 19 septembre sur les routes Office (refus entre 4 412 819 et 4 517 676 octets), frontière affinée le 20/09 : 4 493 821 octets acceptés, 4 493 924 refusés.** Ces routes envoient du **multipart brut**. En revanche, tout outil qui encode en **base64** touche le mur dès **3,3 Mo de fichier réel** (inflation ×1,33) — c'est ce qui est arrivé deux fois. **Tout nouvel outil qui fait transiter un fichier par Vercel doit être conçu en le sachant.**
...
| **16** | **ffmpeg natif — service Railway `media-processing`** | **Serveur** | **2** (`video-compressor`, `video-converter`) — depuis le 20/09 |
