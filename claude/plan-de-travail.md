...
> **DÉCISION du 20 septembre :** `video-compressor` et `video-converter` (moteur `captureStream` + MediaRecorder, temps réel, sortie WebM/MP4 selon le navigateur, **inutilisables sous Safari**) sont rattachés ici : passage sur ffmpeg.wasm chiffré **14-20 h** (compressor 6-8 h, converter 8-12 h), après le lancement. `video-trimmer` est déjà passé sur ffmpeg.wasm. Mesure : moteur ~10 Mo sur le réseau, coupe sans ré-encodage 0,2-0,9 s ; un ré-encodage x264 reste ~1× le temps réel, donc mobile limité.
...
| S1 | `video-compressor` / `video-converter` / `video-trimmer` : `captureStream()` absent de `<video>`, MediaRecorder sans WebM | Moyenne — panne franche | 🟡 `video-trimmer` **refait sur ffmpeg.wasm** (retest Safari requis) ; compressor et converter : message AVANT usage, **DÉCIDÉ 20/09 : après le lancement, bloquant 6** |
