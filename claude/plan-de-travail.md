...
| 15 | zip-extractor | ZIP seul | RAR/7z/ZIP chiffré (libarchive WASM), « tout télécharger » | 4-6 h | 🟡 **EN COURS, commité sur `licence-ameliorations`, PAS déployé** — état de reprise ci-dessous |
...
> **Amélioration 15 — état exact de reprise (arrêt du 25/09).**
> **Moyen de la référence (ezyZip, lu dans son réseau le 24/09) :** 7-Zip compilé en WASM + zip.js + libarchive.js, OPFS pour les gros fichiers ; il **liste d'abord puis extrait à la demande** (un fichier au clic sur « Save »). Plafond gratuit annoncé : 2 Go d'archive.
> **Ce qui est construit** (`app/tools/file-tools/zip-extractor/` : `page.jsx`, `extract.worker.js`, `config.js`) : Worker à session — message `open` (liste) puis `get` (extrait seulement les chemins demandés). **ZIP → zip.js** (BSD-3 : noms CP437 corrects, AES/ZipCrypto, `.z01` découpés, Zip64) ; **tout le reste → 7-Zip 24.09** (`public/wasm/7zz.es6.js` + `7zz.wasm` servis non modifiés, licence LGPL + unRAR dans `public/wasm/7zz-LICENSE.txt`, importés hors bundle) ; archive montée en WORKERFS (lue par morceaux, jamais copiée) ; `.tar.gz` & co déballés en deux temps ; mot de passe demandé à l'ouverture ou à l'extraction ; « Download all as ZIP » par lots de 256 Mo (client-zip, MIT), **écrit en flux sur disque via `showSaveFilePicker`** sous Chrome/Edge, Blob en mémoire ailleurs (≤ 1,9 Go) ; « Save all to a folder » (Chrome/Edge) ; annulation.
> **Ce qui est prouvé :** la **version précédente** (tout extraire d'abord) passait **20/20 sous Chromium ET Firefox** sur `scripts/browser-tests/zip-extractor.mjs` (RAR WinRAR aux noms chinois avec CRC de WinRAR, RAR5 chiffré 3 volumes, 7z AES noms chiffrés, ZIP AES-256 et ZipCrypto, 7z découpé complet/incomplet, `.z01`, CAB/LZH/7z-bzip2 de libarchive = extraction de `tar.exe`, non-archive, ZIP global rouvert). Elle a été **abandonnée parce que mesurée EN DESSOUS de la référence** : même RAR de 1,99 Go (Chromium), **22,0 s** jusqu'au premier fichier contre **6,2 s** chez ezyZip ; et **Firefox figé sans erreur à 2,8 Gio** tenus en mémoire. Mesures qui restent valables : 1,86 Gio extraits → téléchargements intacts dans les deux navigateurs ; ZIP global en Blob OK sous Firefox (23 s), **« Failed to fetch » sous Chromium** (d'où l'écriture en flux).
> **La nouvelle version (extraction à la demande) n'a PAS encore passé ses tests.** Premier passage Chromium : 3 PASS, puis `HeaderEnc1234.rar` en FAIL — **l'erreur est dans le TEST, pas dans l'outil** : vérifié à la main, la page affiche bien « This archive is password-protected ». Cause : `waitState()` du test a conclu « listé » trop tôt (le sélecteur `[data-status]` résolu sur un état précédent / en double, erreur « strict mode violation: 2 elements »).
> **Reste à faire, dans l'ordre :**
> 1. corriger la détection de fin de `waitState()` dans `scripts/browser-tests/zip-extractor.mjs` (attendre un état lié au fichier qu'on vient de choisir, `.first()` ou vider le statut à chaque choix) ;
> 2. faire passer toute la suite sous **Chromium** (`node scripts/browser-tests/archive-fixtures.mjs <dossier>` d'abord — WinRAR 7.10 installé sur ce poste fournit RAR5 et `.z01` —, puis `node scripts/browser-tests/zip-extractor.mjs http://localhost:3100 <dossier>`) ;
> 3. la même sous **Firefox** (`--browser=firefox`) ;
> 4. mesurer le **plafond par fichier** annoncé à 1,9 Go (`MAX_FILE_BYTES`, **non mesuré** : seuls des fichiers de 950 Mio l'ont été) avec un seul fichier de ~1,9 Go, `scripts/browser-tests/zip-extractor-cap.mjs`, Chromium + Firefox ; abaisser si ça échoue ;
> 5. **nouvelle comparaison avec ezyZip** sur le RAR de 1,99 Go (`scripts/browser-tests/zip-extractor-vs-ezyzip.mjs <origine> a.bin <source de a.bin> <archive>`), cible ≤ 6,2 s, contenu identique ;
> 6. préversion (lien par le connecteur Vercel), mêmes tests ;
> 7. production (avance rapide de `master`) ;
> 8. vérification sur www avec les vrais fichiers.
> Fichiers d'essai et archive de 1,99 Go : dans le dossier temporaire de la session du 24/09 (à régénérer au besoin : `archive-fixtures.mjs`, et `Rar.exe a -m0 -ma5` sur deux fichiers aléatoires de 950 Mio).
