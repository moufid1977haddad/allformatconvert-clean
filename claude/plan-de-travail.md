...
- **⚠️ Stockage de déploiement Vercel — mesuré en direct le 22/09, PIRE que le chiffre du courriel malgré le nettoyage.** Tableau de bord Vercel (Usage → Deployment Storage, lu par Claude in Chrome, projet `onlineconvertools` seul sur l'équipe) : **7,79 Go / 10 Go = 77,9 %**, contre 75 % annoncés par le courriel — **la marge s'est réduite, pas agrandie**, alors que les déploiements sont passés de 207 à 5 (puis 6, un nouveau créé par un commit poussé juste après le nettoyage — revérifié à l'instant : `dpl_Nd7PuWVhZB2TLWdR7gZGjwDt6vT8`, CANCELED, 22/09 03h08). **Explication la plus probable, pas prouvée à 100 % :** cette métrique ne compte pas le nombre de déploiements mais la taille des artefacts de build **actuellement conservés** ; les 202 supprimés étaient anciens et probablement déjà recyclés par Vercel lui-même (courant sur les plateformes cloud), donc leur suppression explicite n'a rien changé à ce chiffre. Le vrai poids, ce sont les **5-6 déploiements gardés pour le retour arrière**, chacun avec son propre build complet (≈ 1,3 Go/déploiement en moyenne sur les 6 actuels — estimation, pas une mesure par déploiement, l'API ne l'expose pas). **Marge restante : 2,21 Go (22,1 %).** **Rythme observé sur le graphique 30 jours** : 4,68 Go le 23/08 → 7,79 Go le 22/09, avec une accélération nette la dernière semaine (6,0 Go le 14/09 → 7,79 Go le 22/09, soit ≈ 0,26 Go/jour, contre ≈ 0,11 Go/jour en moyenne sur tout le mois) — corrélée aux chantiers récents à gros volume (D8, vidéo, Gotenberg). **À ce rythme récent, le palier des 10 Go serait atteint dans environ 8 à 9 jours si rien ne change.** **Le vrai levier n'est donc pas de supprimer d'anciens déploiements** (déjà fait, sans effet mesurable ici) **mais de réduire le nombre de déploiements gardés en marge de retour arrière** (5 → 2-3) et/ou la fréquence des déploiements de production pendant les chantiers actifs.
- **✅ Search Console — les 7 pages « de notre faute », URL relevées et diagnostiquées le 22/09 par Claude in Chrome (Indexation → Pages → chaque catégorie).** Aucune des 7 n'est cassée aujourd'hui ; détail par catégorie :
  ...
  **Aucune URL saine retirée du sitemap ni passée en `noindex`.** Aucun clic sur « Valider la correction » n'a été fait — la règle NE PAS TOUCHER du plan l'interdit sans ton accord ; pour les 3 « 404 », la correction existe bel et bien depuis le 31/08, donc valider serait légitime **si tu es d'accord**.
