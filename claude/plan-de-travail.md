...
- **✅ Sort de `gotenberg-fonts` — indépendance de `gotenberg-v2` PROUVÉE le 22/09, suppression EN ATTENTE de l'accord du propriétaire** (`RAPPORT-gotenberg-independance.md`). Les 8 variables encore référencées de `gotenberg-v2` (le 9ᵉ, `API_TIMEOUT`, était déjà propre depuis le passage à 240 s le même jour) ont été copiées en valeurs propres sans jamais être lues ni affichées (copie serveur à serveur via l'API GraphQL, seule la longueur en caractères a été journalisée) ; `gotenberg-v2` a été redéployé et vérifié en production avec de vrais fichiers sur 5 des 6 outils qui en dépendent (`excel-to-pdf`, `ppt-to-pdf`, `html-to-pdf`, `epub-to-pdf`, `mobi-to-pdf`) ; **tous passent**. Le 6ᵉ (`word-to-pdf` sur son repli `.doc`) partage le code déjà prouvé (`handleGotenberg`) mais n'a pas eu son propre test faute d'un vrai fichier `.doc` trouvé rapidement. **`gotenberg-fonts` confirmé : il ne dort pas** — veille Serverless désactivée, tourne 24 h/24, zéro requête depuis le 19/09 ; le coût mesuré (≈ 0,7-5,7 $/mois) est donc réel et continu. **Rien n'a été supprimé : la décision reste au propriétaire.**
...
- 🔴 **AVANT Product Hunt — signer le DPA ConvertAPI** *(décision du propriétaire, 20/09/2026 ; urgence accrue le 22/09 : les `.docx` et PDF jusqu'à 100 Mo transitent maintenant par ConvertAPI, voir `RAPPORT-plafonds-mesures.md`)*. **✅ Procédure réelle trouvée le 22/09** (`RAPPORT-gotenberg-independance.md` §4, document légal lu directement, pas un résumé de seconde main) : `help.convertapi.com` reste injoignable depuis ce poste, mais `convertapi.com` répond. Le document `https://www.convertapi.com/compliance/dpa.pdf` (« Privacy Policy and Data Processing Terms », 7 février 2025) dit noir sur blanc que ses conditions s'appliquent **déjà, automatiquement, sans signature** — un DPA individuel signé est une option sur demande, pas une condition de protection. **Contact officiel confirmé dans le document : privacy@convertapi.com** (DPO ConvertAPI, UAB, Vilnius, Lituanie). **Étapes (~15 min, à faire par le propriétaire) :** ① se connecter au tableau de bord ConvertAPI, vérifier le palier tarifaire (Startup/49 $ et au-dessus inclut « Signed NDA & DPA », non vérifié pour ce compte) ; ② chercher une section « Contracts »/« Legal » dans le tableau de bord (signalée par une source indirecte, à confirmer à l'écran) ; ③ à défaut, écrire à **privacy@convertapi.com** en citant le document du 7 février 2025 et en demandant le DPA individuel signé, avec le nom exact de l'entité cliente (doit être identique à celui du compte) ; ④ conserver la copie signée, la déposer dans « Contracts » si elle existe ; ⑤ noter la date ici.
