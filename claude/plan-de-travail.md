...
> **Fait le 21/09 :** les routes `convert-to-pdf` (Word, Excel, PowerPoint), `convert-html-to-pdf` (HTML, EPUB, MOBI), `pdf-to-word`, `pdf-repair`, `pdf-to-pdfa` et `ai-transcribe` (audio) passent par l'envoi par morceaux (service : type de job `stage`, billet « serveur » distinct ; fusionné sur `master` en premier). **Plafonds MESURÉS avec de vrais fichiers dans un vrai navigateur :** Word et PowerPoint **148 Mio réussis, 198 Mio échouent (mémoire de la fonction Vercel)** → **annoncé 100 Mo** ; **Excel 15 Mo** (Gotenberg `API_TIMEOUT` = 60 s : 17,6 Mio passent en 51 s, 24,7 Mio sont coupés) ; **PDF→Word 60 Mo** ; **HTML/EPUB/MOBI 30 Mo** ; **PDF Repair / PDF/A 30 Mo** ; **audio 25 Mo** (limite de Whisper). Petits fichiers : le nouveau chemin est plus lent de 1 à 6 s → **l'ancien chemin est gardé jusqu'à 4 Mio** (`OFFICE_STAGED_THRESHOLD_BYTES`). **Textes de plafond corrigés une seconde fois** (FAQ de catégorie, FAQ d'outils, aides sous les champs). **Non fait :** `image-captioner` (redimensionnement navigateur), Excel au-delà de 15 Mo (décision du propriétaire sur `API_TIMEOUT` de Gotenberg de production, recommandé 180-240 s puis retest), Word/PowerPoint au-delà de ~150 Mo (mémoire de la fonction Vercel), Safari réel.
...
| **AVANT (décision du 20/09)** | ~~D8~~ ✅ fait le 21/09 (voir 2 bis) · **signer le DPA ConvertAPI** (les `.docx` jusqu'à 100 Mo transitent maintenant par ConvertAPI) · **trancher la saturation (dimensionnement du service ffmpeg et de Gotenberg) avant Product Hunt** · **décider `API_TIMEOUT` de Gotenberg (60 s → 180-240 s) pour relever le plafond Excel** |
...
**Bloquants avant le lancement : 9 (Safari — macOS mesuré, iPhone et retest des correctifs à faire par le propriétaire) ; ~~D8 (plafond de ≈ 4,5 Mo)~~ ✅ fait sur préversion le 21/09 (mise en production : voir le rapport) ; la saturation à trancher avant Product Hunt ; le DPA ConvertAPI (à signer par le propriétaire).** Railway Gotenberg à 3 réplicas, et la galerie Product Hunt.
