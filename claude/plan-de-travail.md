...
- **✅ Stockage de déploiement Vercel nettoyé le 22/09** (`RAPPORT-stockage-et-indexation.md`) — courriel Vercel signalant 75 % du palier gratuit (10 Go). **207 déploiements ramenés à 5** (production actuelle + 4 de marge — le détail et la justification du nombre gardé sont dans le rapport). Production actuelle jamais touchée, vérifiée intacte avant et après. **Le pourcentage réel après nettoyage n'a pas pu être relu par l'agent (aucun outil ne l'expose) — à vérifier dans le tableau de bord ou au prochain courriel.**
- 🔴 **Search Console — 7 pages « de notre faute » à corriger, URL précises manquantes.** Rapport du 17/09 : 3 « Introuvable (404) », 3 « Page avec redirection », 1 « Erreur liée aux redirections ». Vérification en direct le 22/09 (240 URL du sitemap, 202 redirections codées, configuration de domaine) : **zéro anomalie trouvée** — donc probablement des URL historiques hors de la table de redirections actuelle, ou déjà corrigées depuis le 17/09. **Aucun outil ne donne accès à la liste précise des URL depuis Search Console** : à copier depuis Search Console → Pages → chaque catégorie, puis transmettre pour correction ciblée et vérifiée.
...
| Quand | Quoi |
|---|---|
| ✅ **Déclenché le 22/09 — le rapport Indexation a dépassé le 03/09, relevé au 17/09/2026** | **45 pages indexées sur 248, 203 non indexées.** Répartition des 203 (`RAPPORT-stockage-et-indexation.md`) : **192 « Détectée, actuellement non indexée »** — autorité de domaine, ne se corrige pas par du code, cohérent avec la position moyenne 74,7 déjà documentée ; **7 imputables au site** — 3 « Introuvable (404) », 3 « Page avec redirection », 1 « Erreur liée aux redirections ». **Vérifié en direct le 22/09, zéro anomalie trouvée** : les 240 URL du sitemap répondent 200, les 202 redirections codées (`lib/legacyRedirects.ts`) sont toutes saines (un seul saut, jamais de chaîne), la configuration de domaine est standard. **Les URL précises des 7 pages restent à obtenir** (Search Console → Pages → cliquer chacune des trois catégories) — aucun outil ne donne à l'agent un accès direct à Search Console pour les lister lui-même. **À transmettre pour correction ciblée.** Aucune URL saine retirée du sitemap ni passée en `noindex`. |
