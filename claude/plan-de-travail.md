...
- 🔴 **AVANT Product Hunt — signer le DPA ConvertAPI** *(décision du propriétaire, 20/09/2026 : le lancement amène des visiteurs européens ; ConvertAPI reçoit les `.docx` de `word-to-pdf` et `pdf-to-word`, et la politique de confidentialité le cite déjà comme sous-traitant)*. **Concrètement (~15 min, à faire par le propriétaire — c'est une signature) :** ① se connecter au tableau de bord ConvertAPI du compte utilisé en production ; ② **y télécharger le DPA** (il se récupère dans le tableau de bord) ; ③ **le signer au nom de la même entité que celle qui est partie au contrat principal ConvertAPI** (le nom du client du DPA doit être identique à celui du compte, sinon il n'est pas valable) ; ④ **le téléverser au même endroit** du tableau de bord (la même section permet de le résilier et de le re-signer plus tard) ; ⑤ conserver la copie signée ; ⑥ noter la date ici. *Source : article d'aide ConvertAPI « How to sign my GDPR Data Processing Agreement (DPA)? » ; la page n'était pas joignable depuis ce poste le 20/09, les étapes viennent de l'extrait de recherche et sont à confirmer à l'écran.*
...
| **AVANT (décision du 20/09)** | **D8 : relever le plafond de ≈ 4,5 Mo** en branchant les routes Office sur l'envoi par morceaux (8-12 h) · **signer le DPA ConvertAPI** |
...
**Bloquants avant le lancement : 9 (Safari — macOS mesuré, iPhone et retest des correctifs à faire par le propriétaire) ; D8 (plafond de ≈ 4,5 Mo, 8-12 h — décision du 20/09) ; le DPA ConvertAPI (à signer par le propriétaire).** Railway Gotenberg à 3 réplicas, et la galerie Product Hunt.

Le bloquant 2 est **mesuré et ses promesses corrigées en ligne** ; ses défauts résiduels (D7, D9) passent après le lancement ; **D8 (plafond) passe AVANT** (décision du propriétaire, 20/09).
