...
// HTML / EPUB / MOBI to PDF (one self-contained HTML): largest measured is 30 MiB (12.9 s, 22.5 MB PDF).
...
// PDF to Word (ConvertAPI): measured 100.3 MB pass (204.6 s, 130 MB .docx out); 150.4 MB fails --
// `Vercel Runtime Error: instance was killed because it ran out of available memory` (read from the runtime
// log, not guessed). This is an ARCHITECTURE ceiling: the function holds the PDF in AND the .docx out at
// once. It is expected to move if/when this path is rebuilt like the video service (stream instead of buffer).
const MAX_PDF_TO_WORD_STAGED_BYTES = 100 * 1024 * 1024;
...
// veraPDF themselves). Measured: 45.4 MB passes (22-30 s) on both tools; 60.4 MB and 100.3 MB are refused by
// that service with "Maximum size is 50 MB.". ARCHITECTURE-ish: raising the service's own env var would raise
// this without any code change, but that is a separate decision (a second Railway service to touch).
const MAX_PDFTOOLS_STAGED_BYTES = 45 * 1024 * 1024; // largest measured (30.4 MB repaired in 15 s)
// Image Captioner: the ORIGINAL image the visitor selects. It is decoded and reduced to a 2048 px JPEG in the browser
// (app/lib/imageForVision.js), so only that small copy travels. Set from measurement (see RAPPORT-plafonds-mesures.md).
