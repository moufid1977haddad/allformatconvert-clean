...
// PDF Compress on the pdf-tools service (docs/audit/RAPPORT-ecarts-marche.md §3a). The file goes browser ->
// media service -> pdf-tools directly, never through a Vercel function, so the function's memory is not the
// ceiling. 200 MiB = iLovePDF's free limit for Compress. Measured locally on a 200 MiB, 94-page photo PDF:
// 32.5 s / 217 MiB peak (recommended), 26.3 s / 158 MiB (extreme); to be re-measured on Railway (see report).
// Above it the page keeps its in-browser, structure-only optimisation (app/tools/pdf-tools/pdf-compress/config.js).
const MAX_PDF_COMPRESS_STAGED_BYTES = 200 * 1024 * 1024;
...
module.exports = {
  MAX_IMAGE_CAPTIONER_ORIGINAL_BYTES, MAX_HTML_STAGED_BYTES, MAX_PDF_TO_WORD_STAGED_BYTES, MAX_SPREADSHEET_STAGED_BYTES, MAX_PDFTOOLS_STAGED_BYTES, MAX_PDF_COMPRESS_STAGED_BYTES, MAX_AUDIO_STAGED_BYTES, MAX_OFFICE_STAGED_BYTES, OFFICE_STAGED_THRESHOLD_BYTES,
