...
const CONVERTAPI_DOCX_TO_PDF_URL = 'https://v2.convertapi.com/convert/docx/to/pdf';
// Reverse direction, added for the PDF-to-Word tool -- same host, same
// auth, same StoreFile/response contract, just the other verb pair.
const CONVERTAPI_PDF_TO_DOCX_URL = 'https://v2.convertapi.com/convert/pdf/to/docx';
// PDF to Excel / PowerPoint (2026-09-23): same host, auth and contract. Both endpoints were read on
// ConvertAPI's own pages on 2026-09-22 (OCR included) -- docs/audit/RAPPORT-outils-mis-en-avant.md §5.
const CONVERTAPI_PDF_TO_XLSX_URL = 'https://v2.convertapi.com/convert/pdf/to/xlsx';
const CONVERTAPI_PDF_TO_PPTX_URL = 'https://v2.convertapi.com/convert/pdf/to/pptx';
...
const CONVERTAPI_BASE_TIMEOUT_MS = 30_000;
// Grows with the file (a large PDF / document takes ConvertAPI longer); capped under the route's maxDuration.
function convertApiTimeoutMs(bytes) {
  return Math.min(240_000, CONVERTAPI_BASE_TIMEOUT_MS + Math.ceil(bytes / (1024 * 1024)) * 3_000);
}
...
// catch-all. Generic across both conversion directions -- ConvertAPI's
// response-code taxonomy isn't docx-specific.
...
// Shared request/response handling for both conversion directions --
// fetch/timeout/parsing/error-classification is identical either way, only
// the URL and the resulting file's field differ. Pure extraction: the
// docx-to-pdf behavior below is byte-for-byte what convertDocxToPdf did
// before this helper existed.
/**
 * @param {string} url
 * @param {Buffer} fileBuffer
 * @param {string} fileName
 * @returns {Promise<{ buffer: Buffer, costMicros: number }>}
 * @throws {ConvertApiError}
 */
async function callConvertApi(url, fileBuffer, fileName) {
  ...
  const timeoutId = setTimeout(() => controller.abort(), convertApiTimeoutMs(fileBuffer.length));
  ...
  const buffer = Buffer.from(fileEntry.FileData, 'base64');
...
async function convertDocxToPdf(fileBuffer, fileName) {
  const { buffer, costMicros } = await callConvertApi(CONVERTAPI_DOCX_TO_PDF_URL, fileBuffer, fileName);
  return { pdfBuffer: buffer, costMicros };
}

/**
 * @param {Buffer} fileBuffer
 * @param {string} fileName
 * @returns {Promise<{ docxBuffer: Buffer, costMicros: number }>}
 * @throws {ConvertApiError}
 */
async function convertPdfToDocx(fileBuffer, fileName) {
  const { buffer, costMicros } = await callConvertApi(CONVERTAPI_PDF_TO_DOCX_URL, fileBuffer, fileName);
  return { docxBuffer: buffer, costMicros };
}

/** @returns {Promise<{ buffer: Buffer, costMicros: number }>} @throws {ConvertApiError} */
async function convertPdfToXlsx(fileBuffer, fileName) {
  return callConvertApi(CONVERTAPI_PDF_TO_XLSX_URL, fileBuffer, fileName);
}

/** @returns {Promise<{ buffer: Buffer, costMicros: number }>} @throws {ConvertApiError} */
async function convertPdfToPptx(fileBuffer, fileName) {
  return callConvertApi(CONVERTAPI_PDF_TO_PPTX_URL, fileBuffer, fileName);
}

module.exports = { convertDocxToPdf, convertPdfToDocx, convertPdfToXlsx, convertPdfToPptx, ConvertApiError };
