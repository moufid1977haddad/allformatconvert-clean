...
const { WORST_CASE_COST_MICROS, actualAiCostMicros, actualAiTranscribeCostMicros, GLOBAL_SPEND_CAP_MICROS } = require('../../lib/quota/config');
...
assert.ok(WORST_CASE_COST_MICROS['ai-transcribe'] < GLOBAL_SPEND_CAP_MICROS / 10,
  `a single worst-case transcribe reservation (${WORST_CASE_COST_MICROS['ai-transcribe']} micros) should allow at least 10 calls/month under the cap (${GLOBAL_SPEND_CAP_MICROS} micros)`);

assert.strictEqual(actualAiCostMicros({ prompt_tokens: 0, completion_tokens: 0 }), 0);
assert.ok(actualAiCostMicros({ prompt_tokens: 1000, completion_tokens: 1000 }) > 0);
assert.ok(actualAiTranscribeCostMicros(120) > 0);

console.log('PASS: config.js cost formulas and limits.js client checks.', WORST_CASE_COST_MICROS);
