...
if (process.argv.includes('--batch')) {
  ...
  if (SITES.includes('bm')) { await bm.open('Ean13'); await run('bm', async (p) => { await p.locator('textarea#input').fill(values.join('\n')); await p.waitForTimeout(3000); await p.locator('button').filter({ hasText: /^\s*(PNG|JPG|GIF|SVG)\s*$/ }).last().click(); await p.getByRole('menuitem', { name: 'PNG', exact: true }).or(p.getByRole('option', { name: 'PNG', exact: true })).first().click(); await p.locator('button').filter({ hasText: 'Download' }).first().click(); }); }
}
