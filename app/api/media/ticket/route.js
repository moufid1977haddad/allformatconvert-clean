...
export async function POST(req) {
  ...
  const v = validateRequest(body, cfg.maxBytes);
  ...
  try {
    const hour = await incrementCounter(`media_rate:hour:${hash}`, hourKey, 1, cfg.perHour);
    if (!hour.allowed) {
    ...
    const day = await incrementCounter(`media_rate:day:${hash}`, dayKey, 1, cfg.perDay);
    if (!day.allowed) {
      await decrementCounter(`media_rate:hour:${hash}`, hourKey, 1);
  ...
  const t = mintTicket({ secret: cfg.secret, op: v.op, maxBytes: cfg.maxBytes });
