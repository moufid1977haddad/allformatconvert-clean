...
function StatCard({ value, label, suffix, icon, animate }) {
  ...
  return (
    <div style={{ background:'#ffffff', border:'1px solid #e2e8f0', borderRadius:'16px', padding:'20px 24px', textAlign:'center' }}>
      <div style={{ fontSize:'28px', marginBottom:'4px' }}>{icon}</div>
      <div style={{ fontSize:'32px', fontWeight:'800', color:'#0f172a', lineHeight:1 }}>{animate ? count : value}{suffix}</div>
      <div style={{ fontSize:'12px', color:'#64748b', marginTop:'4px', letterSpacing:'0.05em', textTransform:'uppercase' }}>{label}</div>
    </div>
  );
}
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-neutral-100">
      ...
      {/* ═══ HERO ═══ */}
      <section style={{ background:'#f8fafc', padding:'80px 24px 60px', position:'relative', overflow:'hidden' }}>
        <div style={{ position:'absolute', inset:0, overflow:'hidden', pointerEvents:'none' }}>
          <div style={{ position:'absolute', top:'-80px', right:'-80px', width:'400px', height:'400px', background:'rgba(226,232,240,0.5)', borderRadius:'50%', filter:'blur(60px)' }} />
          <div style={{ position:'absolute', bottom:'-100px', left:'-60px', width:'350px', height:'350px', background:'rgba(226,232,240,0.3)', borderRadius:'50%', filter:'blur(80px)' }} />
        </div>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', gap:'48px', alignItems:'center', position:'relative', zIndex:1 }}>
          ...
          <div style={{ flex:1, animation:'fadeUp 0.7s ease both' }}>
            <div style={{ display:'inline-flex', alignItems:'center', gap:'8px', background:'#e2e8f0', border:'1px solid #cbd5e1', borderRadius:'999px', padding:'6px 16px', marginBottom:'24px', fontSize:'13px', color:'#475569' }}>
              ✨ 232+ free tools · No signup · No limits
            </div>
            <h1 style={{ fontSize:'clamp(36px,5vw,58px)', fontWeight:'800', color:'#0f172a', lineHeight:'1.1', marginBottom:'20px', letterSpacing:'-0.02em' }}>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:'10px', marginBottom:'40px' }}>
              {badges.map(b => (
                <span key={b.text} className="badge-pill" style={{ display:'flex', alignItems:'center', gap:'6px', background:'#f1f5f9', border:'1px solid #e2e8f0', borderRadius:'999px', padding:'7px 14px', fontSize:'13px', color:'#475569', transition:'background 0.2s' }}>
                  {b.icon} {b.text}
                </span>
              ))}
            </div>
            <div style={{ display:'flex', gap:'12px', flexWrap:'wrap' }}>
              <Link href="/tools/pdf-tools" style={{ background:'#6366f1', color:'#fff', padding:'14px 28px', borderRadius:'12px', fontWeight:'700', fontSize:'15px', textDecoration:'none', display:'inline-block', boxShadow:'0 4px 20px rgba(99,102,241,0.2)' }}>🚀 Explore PDF Tools</Link>
              <Link href="/tools/ai-tools"  style={{ background:'#f1f5f9', color:'#1e293b', border:'1px solid #e2e8f0' }}>🤖 Try AI Tools</Link>
            </div>
          </div>
          ...
          <div style={{ flex:'0 0 360px', display:'grid', gridTemplateColumns:'repeat(3, 110px)', gap:'12px', justifyContent:'center', animation:'fadeUp 0.9s ease 0.2s both' }}>
            {floatingIcons.map(item => (
              <div key={item.label} style={{ width:'110px', height:'88px', background:item.color, border:`1.5px solid ${item.border}`, borderRadius:'18px', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:'6px', animation:`${item.anim} ${item.dur} ease-in-out ${item.delay} infinite`, boxShadow:'0 6px 20px rgba(0,0,0,0.10)' }}>
                <span style={{ fontSize:'28px' }}>{item.icon}</span>
                <span style={{ fontSize:'11px', fontWeight:'600', color:'#374151' }}>{item.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ═══ STATS ═══ */}
      <section ref={statsRef} style={{ background:'#f1f5f9', padding:'48px 24px' }}>
      ...
      {/* ═══ QUOTE ═══ */}
      <section style={{ background:'#fff', padding:'56px 24px', textAlign:'center', borderBottom:'1px solid #e2e8f0' }}>
        <div style={{ maxWidth:'720px', margin:'0 auto' }}>
          <div style={{ fontSize:'48px', marginBottom:'12px', lineHeight:1, color:'#9ca3af' }}>"</div>
          <p style={{ fontSize:'clamp(18px,2.5vw,24px)', fontWeight:'600', color:'#1e1b4b', lineHeight:'1.6', marginBottom:'20px', fontStyle:'italic' }}>
            The fastest way to convert, compress, and transform your files — 232 tools, zero installation, completely free.
          </p>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'12px' }}>
            <div style={{ width:'40px', height:'2px', background:'#9ca3af' }} />
            <span style={{ fontSize:'13px', color:'#9ca3af', fontWeight:'500', letterSpacing:'0.05em', textTransform:'uppercase' }}>AllFormatConvert · Trusted in 190+ countries</span>
            <div style={{ width:'40px', height:'2px', background:'#9ca3af' }} />
          </div>
        </div>
      </section>
      ...
      <section style={{ background:'#f8fafc', padding:'72px 24px' }}>
        <div style={{ maxWidth:'900px', margin:'0 auto', textAlign:'center' }}>
          ...
          <h2 style={{ fontSize:'clamp(28px,4vw,42px)', fontWeight:'800', color:'#0f172a', marginBottom:'16px', letterSpacing:'-0.02em' }}>Powered by AI</h2>
          <p style={{ fontSize:'18px', color:'#64748b', marginBottom:'40px', maxWidth:'580px', margin:'0 auto 40px', lineHeight:'1.7' }}>
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:'12px', justifyContent:'center', marginBottom:'40px' }}>
            {['✨ Grammar Fixer','🌍 AI Translator','📝 Text Summarizer','🎨 Background Remover','🔍 Image Upscaler','🎙️ Audio Transcriber','💬 AI Chatbot','📧 Email Generator'].map(t => (
              <span key={t} style={{ background:'#e2e8f0', border:'1px solid #cbd5e1', borderRadius:'999px', padding:'8px 18px', fontSize:'13px', color:'#334155', fontWeight:'500' }}>{t}</span>
            ))}
          </div>
      ...
      {/* ═══ FOOTER STRIP ═══ */}
      <section style={{ background:'#fff', padding:'32px 24px', textAlign:'center', borderTop:'1px solid #e2e8f0' }}>
