  ...
  return (
    <div className="min-h-screen bg-neutral-100 p-6">
      <div className="max-w-2xl mx-auto">
        ...
        <p className="text-neutral-500 text-center mb-8">Adjust audio frequencies</p>
        <div className="bg-white border border-neutral-200 rounded-xl shadow-sm p-6 space-y-4">
          ...
          {['bass','mid','treble'].map(band => (
            <div key={band}>
              <label className="block text-sm text-neutral-500 mb-1 capitalize">{band}: {bands[band]}dB</label>
              <input type="range" min="-12" max="12" value={bands[band]} onChange={e => setBands(p => ({...p, [band]: parseInt(e.target.value)}))} className="w-full" />
            </div>
          ))}
          <button onClick={apply} disabled={!file || loading} className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:bg-neutral-200 rounded-xl py-3 font-semibold transition">{loading ? 'Processing...' : 'Apply EQ'}</button>
